...
- **PHPUnit test suite** (`tests/`): Bootstrap with WordPress function stubs + skeleton tests for `SecretsVault` (encrypt/decrypt round-trip, tamper detection), `Settings` (type/range/enum/bool/array validation), and `Database` query validator (comment stripping, keyword denylist, conditional-comment bypass). `phpunit.xml` config and `composer run test` script added.
- **Dev tooling** (`composer.json`): Added `phpunit/phpunit ^10.5`, `phpstan/phpstan ^1.12`, `wp-coding-standards/wpcs ^3.1`, and `phpcompatibility/phpcompatibility-wp` as dev dependencies. `composer run lint` runs PHPCS (WordPress standard) + PHPStan (level 6) on `includes/`. `composer run test` runs the test suite.
...
- **`API/Database.php` — MySQL conditional-comment bypass closed**: `validate_select_query()` previously validated the comment-stripped SQL but executed the original. A MySQL conditional comment such as `SELECT /*!50000 INTO OUTFILE …*/ 1` would pass the keyword denylist check but have its dangerous content executed. Now a dedicated `strip_sql_comments()` method strips both `--` and `/* */` / `/*!…*/` comments, and both the validator and the database receive that same cleaned string, eliminating the divergence.
- **`Content/VersionManager.php` — DELETE IN uses prepared statement**: The bulk version-cleanup query now uses `$wpdb->prepare()` with `%d` placeholders instead of raw `intval()`-joined string interpolation.
- **`API/EmailMarketing.php` — `SHOW TABLES LIKE` uses `$wpdb->prepare()`**: Two instances of `"SHOW TABLES LIKE '{$table}'"` (string interpolation) replaced with `$wpdb->prepare('SHOW TABLES LIKE %s', $table)` to match the prepare-everywhere standard.
- **`API/SEO.php` — `SHOW TABLES LIKE` uses `$wpdb->prepare()` (5 locations)**: All redirect-manager table-existence checks converted from interpolation to `$wpdb->prepare()`.
- **`Agent/AgentRuntime.php` — `load_running_agents()` adds LIMIT 500**: The unbounded `SELECT * … WHERE status IN ('running', 'paused')` could load an unbounded number of rows into memory on startup; capped at 500.
- **`Integration/IntegrationManager.php` — `load_integrations()` adds LIMIT 1000**: The full-table `SELECT *` loaded all integrations unconditionally; capped at 1000.
- **`Execution/GoalExecutor.php` — ExecutionLedger always finalised**: Wrapped the entire action-execution loop in `try/catch (\Throwable)` so that an unexpected exception escaping from `check_conditions()` or `AuditLog::log()` no longer leaves the `execution_ledger` row permanently in `running` status (orphaned record) — `complete_execution()` is called and the exception is re-thrown.
- **`Hosting/TunnelManager.php` — binary path confinement check**: Before invoking the `cloudflared` binary, `ensure_binary()` now verifies via `realpath()` that the resolved path is confined within `$this->work_dir`, guarding against symlink-based path traversal attacks.
